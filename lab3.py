...
def get_api_key(filename):
    return main_functions.read_from_file(filename)["aqi_api_key"]

aqi_key = get_api_key("api_key.json")
...
def index():
    form = AQIForm()  # Creates an instance of the AQIForm
    if request.method == "POST":
        parameter_entered = form.parameter.data  # Get the data from the form
        all_data = get_aqi_data(aqi_key,parameter_entered)
        return render_template('aqi_results.html', all_data=all_data)  # Return the results page with the obtained data
    return render_template('aqi.html', form=form)  # Render the main page with the form
